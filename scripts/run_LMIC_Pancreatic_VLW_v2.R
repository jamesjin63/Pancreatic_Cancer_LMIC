...
for (ie in ie_values) {
  ...
  if (ie==ie_base) {
    main_t5 <- bind_rows(
      ...
      `VLW (billion constant-2023 USD; scenario range)`=
        fmt_ui(VLW_billion,VLW_scenario_low_billion,
               VLW_scenario_high_billion,d=2),
      `DALYs (thousands; scenario range)`=
        fmt_ui(DALY,DALY_scenario_low,DALY_scenario_high,d=1,s=1000)
    )
      ...
      map_dfr(income_fct,function(g) {
        ...
        bind_rows(map_dfr(c("ETS","ARIMA"),function(m) {
          z <- fit_series(s$D,m,label=g); tibble(series=g,outcome="DALY",model=m,
            Year=2050,point=as.numeric(z$fc$mean[fh]),scenario_low=as.numeric(z$fc$lower[fh,1]),
            scenario_high=as.numeric(z$fc$upper[fh,1]))}),
...
cat("\n================ RECONCILIATION ================\n")
print(recon_tbl)
cat("\nALL COMPLETE. Output:", rel_path(out_root), "\n")
