# Shared runner for Supplementary Tables 1-17.
...
if (!exists("TABLE_NUMBER", inherits = FALSE) ||
    length(TABLE_NUMBER) != 1L || TABLE_NUMBER < 1L || TABLE_NUMBER > 17L) {
  stop("A wrapper must set TABLE_NUMBER to an integer from 1 through 17.")
}
